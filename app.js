...
async function saveEpisode(event) {
  ...
  setBusy(button, true, "保存中");
  try {
    state.dashboard = await request("finishEpisode", payload);
    saveJson(STORAGE_KEYS.dashboardCache, state.dashboard);
    renderDashboard(state.dashboard);
    showToast("这次发作已记录");
  } catch (error) {
    enqueuePending("finishEpisode", payload);
    if (error.status === 401) {
      handleAuthFailure("登录已失效；这次发作已暂存");
    } else {
      showToast("网络不可用，这次发作已先保存到本机");
    }
  } finally {
...
async function bootstrapDashboard(allowCache = true) {
  ...
  try {
    ...
    document.body.classList.remove("locked");
    await flushQueue();
    updateDeviceChip(`可信设备 · 到期 ${formatExpiryLabel(state.tokenExpiry)}`);
  } catch (error) {
    if (error.status === 401) {
...
function bindEvents() {
  ...
  dom.episodeFinishForm.addEventListener("submit", saveEpisode);
